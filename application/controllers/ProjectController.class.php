...
/**
    ...
    function initial_list_projects()
    {
        ...
        $sazanCacheRes = sazanIsCached('initial_list_projects', $inData, 60 * 60 * 24);
        if ($sazanCacheRes['status']) {
            $ws = $sazanCacheRes['result'];
            $timeDiff = 'exists ';
            $timeDiff .= microtime(1) - $startTime;
        } else {
            $all_ws = logged_user()->getWorkspaces(true);
            if (!is_array($all_ws)) $all_ws = array();
            ...
            $timeDiff = 'from db ';
            $timeDiff .= microtime(1) - $startTime;
            sazanCache('initial_list_projects', $inData, $ws);
        }
        sazanCacheTime('initial_list_projects', $inData, $timeDiff);
        //sazan End
        ajx_extra_data(array('workspaces' => $ws));
    }
