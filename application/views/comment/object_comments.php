...
function moderator($Data) {
  $userid = $_REQUEST["http___pm2_citrt_net_id"];
    $moderator_button = "<iframe src=\"/plugins/EDUHELPPublish.php?commentid=".$Data."&userid=".$userid."\" scrolling=\"no\" frameborder=\"no\"  width=\"120\" height=\"36\" align='middle'></iframe>";
  return $moderator_button;
}
?>
...
?>
		...
		<?php 	if($comment->getCreatedBy() instanceof User) { ?>
			    ...
			    <div><?=moderator($comment->getId());?><?php echo implode(' | ', $options) ?></div>
		<?php 		} // if ?>
		</td></tr></table>
				</div>
		<?php 	} else { ?>
