...
/*	if($projectid == '12345678') {
	...
	case "2553":
		...
		$active_sheet->setCellValue('F1', 'Классификатор');
                $active_sheet->setCellValue('G1', 'Описание задачи');
                $active_sheet->setCellValue('H1', 'Последний комментарий');
                $active_sheet->setCellValue('I1', 'Создатель задачи');
                $active_sheet->setCellValue('J1', 'Дата создания');
		$active_sheet->setCellValue('K1', 'Исполнитель');
		...
		$active_sheet->getColumnDimension('L')->setWidth(20);
                ...
                $active_sheet->setCellValue('F1', 'Фамилия Имя Отчество');
                $active_sheet->setCellValue('G1', 'Контактный телефон');
		$active_sheet->setCellValue('H1', 'E-mail');
		$active_sheet->setCellValue('I1', 'Фамилия Имя Отчество пользователя');
		$active_sheet->setCellValue('J1', 'Услуга');
		$active_sheet->setCellValue('K1', 'Категория');
		$active_sheet->setCellValue('L1', 'Роль');
                $active_sheet->setCellValue('M1', 'Суть обращения');
                $active_sheet->setCellValue('N1', 'Ход решения');
                $active_sheet->setCellValue('O1', 'Статус');
                $active_sheet->setCellValue('P1', 'Исполнитель');
        break;
	case "12345678":
                $active_sheet->getStyle('A1')->getFont()->setBold(true);
                $active_sheet->getStyle('B1')->getFont()->setBold(true);
...
/*                $active_sheet->getStyle('B1')->getFont()->setBold(true);
		...
		$active_sheet->setCellValue('F1', 'Дата инцидента');
                $active_sheet->setCellValue('G1', 'Суть обращения');
                $active_sheet->setCellValue('H1', 'Ход решения');
		$active_sheet->setCellValue('I1', 'Источник');
		$active_sheet->setCellValue('J1', 'Сфера');
...
/*                $active_sheet->getStyle('A1')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		...
		$active_sheet->setCellValue('L1', 'Дата закрытия');
		$active_sheet->setCellValue('M1', 'Задачу создал');
		$active_sheet->setCellValue('N1', 'Приоритет');
	break;
	}
...
//				if($projectid=="1103") {
							...
							case "2553":
								...
								$active_sheet->setCellValue('F'.$schetchik_excel , $klassificator);
                                                                $active_sheet->setCellValue('G'.$schetchik_excel , $text);
                                                                $active_sheet->setCellValue('H'.$schetchik_excel , $comment);
                                                                $active_sheet->setCellValue('I'.$schetchik_excel , $display_name_creater);
                                                                $active_sheet->setCellValue('J'.$schetchik_excel , $created_on);
								$active_sheet->setCellValue('K'.$schetchik_excel , $display_name);
								$active_sheet->setCellValue('L'.$schetchik_excel , $status);
                                                        break;
							case "5121":
								$schetchik++;
                                                                $schetchik_excel = $schetchik + 1;
								...
								$active_sheet->setCellValue('F'.$schetchik_excel , $fio.$firstname.' '.$name.' '.$fathername);
								$active_sheet->setCellValue('G'.$schetchik_excel , $phone);
								$active_sheet->setCellValue('H'.$schetchik_excel , $email);
								$active_sheet->setCellValue('I'.$schetchik_excel , $fio_user);
								$active_sheet->setCellValue('J'.$schetchik_excel , $usluga);
								$active_sheet->setCellValue('K'.$schetchik_excel , $category);
								$active_sheet->setCellValue('L'.$schetchik_excel , $role);
								$active_sheet->setCellValue('M'.$schetchik_excel , $text);
								$active_sheet->setCellValue('N'.$schetchik_excel , $comment);
                                                                $active_sheet->setCellValue('O'.$schetchik_excel , $status);
                                                                $active_sheet->setCellValue('P'.$schetchik_excel , $display_name);
							break;
							case "12345678":
								if($usluga == 'СМЭВ') {
				                	                $schetchik++;
							...
							case "12345670":
								if((($sphera == 'Личный кабинет на Портале ГМУ РТ')&&($usluga == 'Личный кабинет госслужащего'))||($sphera == 'Конкурс для госслужащих')) {
									$schetchik++;
                                                                        ...
                                                                        $active_sheet->setCellValue('F'.$schetchik_excel , $date_incident.' '.$time_incident);
                                                                        $active_sheet->setCellValue('G'.$schetchik_excel , $text);
                                                                        $active_sheet->setCellValue('H'.$schetchik_excel , $comment);
									$active_sheet->setCellValue('I'.$schetchik_excel , $istochnik);
									$active_sheet->setCellValue('J'.$schetchik_excel , $sphera);
...
//				else {
							...
							default:
								...
								$active_sheet->getCell('B'.$schetchik_excel)->getHyperlink()->setUrl('http://pm.citrt.net/index.php?c=task&a=view_task&id='.$item);
                				                ...
                				                $active_sheet->setCellValue('I'.$schetchik_excel , $status);
			                                        $active_sheet->setCellValue('J'.$schetchik_excel , $display_name);
								$active_sheet->setCellValue('K'.$schetchik_excel , $klassificator.$tip_problemi);
								if($openclose==1) {
									$active_sheet->setCellValue('L'.$schetchik_excel , $completed_on);
								}
								$active_sheet->setCellValue('M'.$schetchik_excel , $display_name_creater);
								$active_sheet->setCellValue('N'.$schetchik_excel , $priority);
							break;
						}
//					}
			}
