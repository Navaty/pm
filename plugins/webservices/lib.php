...
function FOS($id,$lang){
	...
	$dom = new DOMDOcument('1.0','UTF-8');
		...
		$result[$key]['order']=getPriority($dom->getElementsByTagName("priority"));
...
function FOS2($id,$lang){
	...
	$i=1;
        foreach($level as $key => $value){
                @$dom->LoadXML('<form>'.trim($value["description"]).'</form>');
//		if(getMaxim($dom->getElementsByTagName("portal2"))) {
		if(!getMaxim($dom->getElementsByTagName("portal2"))) continue;
	                $result[$i]["id"] = $value["id"];
	                $result[$i]["name"] = fos_bbCodeByLang($value["name"],$lang);
	                $result[$i]['fields']=xmlToArr($dom->getElementsByTagName("form")->item(0),'ru');
			$result[$i]['maxim']=getMaxim($dom->getElementsByTagName("maxim"));
	                $result[$i]['fontSize']=getFontSize($dom->getElementsByTagName("fontSize"));
			$result[$i]['order']=getPriority($dom->getElementsByTagName("priority"));
	                $result[$i]["description"] = fos_bbCodeByLang($description_arr["text"],$lang);
	                $result[$i]["faq"] = (get_notes($value["id"],$lang))?true:false;
	                $description_arr = NULL;
	                if($buff = FOS2($value["id"],$lang))
	                        $result[$i]["childs"] = $buff;
	                $buff = NULL;
	$i++;
//if(!getMaxim($dom->getElementsByTagName("portal2"))) unset($result[$key]);
//		}
        }
        return $result;
}
...
function parseProperty($nodes){
		...
		switch($node->nodeName){
			...
			break;
			case 'required':{
				$out['required']=(bool)trim($node->textContent);
			}break;
			default:{
