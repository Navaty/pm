...
function number_tasks_from_filter($array_tasks)
{
    ...
    $class_error_array = give_me_sons_data($project_id);

    $filter_array = Array();

    if (is_array($array_tasks)) {
        foreach ($array_tasks as $close_task) {
            ...
            $class_error_prop = '';
            $type_problem_prop = '';
            if (is_array($res_object_properties)) { //sazan = добавил проверку Массив ли?
                foreach ($res_object_properties as $item_object_properties) {
                    switch ($item_object_properties["name"]) {
                        case "ТипОшибки":
                            ...
                            break;
                        case "ТипПроблемы":
                            $type_problem_prop = $item_object_properties["value"];
                            break;
                    }
                }
            }
            if (is_array($res_object_properties)) { //sazan = добавил проверка на Массив ли?
                foreach ($class_error_array as $class_error) {
                    $class_error_name = $class_error['name'];
                    $class_error_id = $class_error['id'];
                    $role = $role = bz_pm_get_xml_sub_projects2($class_error_id);
                    $type_problem_array = $role["projects"]["project"];
                    foreach ($type_problem_array as $type_problem) {
                        $type_problem_name = $type_problem["name"];
                        $type_problem_id = $type_problem["id"];
                        if (($class_error_prop == $class_error_name) && ($type_problem_name == $type_problem_prop)) {
//			echo 'Номер задачи: '.$close_task.' Тип ошибки: '.$class_error_name.' Тип проблемы: '.$type_problem_name.'<br />';
//		        $filter_array[$class_error_id] = $class_error_name;
//			$filter_array[$class_error_id][$type_problem_id] = $type_problem_name;
//			var_dump($type_problem_name);
                            /*			$data_about_task["id"] = $close_task;
                                        $data_about_task["class_error"] = $class_error_name;
                                        $data_about_task["type_problem"] = $type_problem_name;*/

                            $filter_array[$class_error_id][$type_problem_id][] = $close_task;
                        } else {
                            //Для проверки
                        }
                    }
...
function count_tasks($tasks_from_filter)
{
    ...
    foreach ($class_error_array as $class_error) {
        ...
        foreach ($type_problem_array as $type_problem) {
            ...
            $statistics["type_problem_id"] = $type_problem_id;
            $statistics["type_problem_name"] = $type_problem_name;
            if (isset($tasks_from_filter[$class_error_id][$type_problem_id])) {
                $statistics["count"] = count($tasks_from_filter[$class_error_id][$type_problem_id]);
                $statistics["tasks"] = $tasks_from_filter[$class_error_id][$type_problem_id];
//		       echo 'Открытых задач по комбинации: '.$class_error_id.' - '.$type_problem_id.' = '.count($tasks_from_filter[$class_error_id][$type_problem_id]).'<br />';
            } else {
//	   	       echo 'Открытых задач по комбинации: '.$class_error_id.' - '.$type_problem_id.' = 0<br />';
                $statistics["count"] = 0;
...
function get_info_about_task($idtask, $current_time)
{ // Получаем полную информацию по работе с задачей
    $taskinfo = Array();
...
function give_me_report_terminal($starttime, $endtime, $project_task_id)
{
    ...
    if (isset($array_tasks_open) && is_array($array_tasks_open)) { //sazan = Проверка на массив и наличие
        $tasks_open = array_unique($array_tasks_open);
    }

//	$array_tasks = array_merge($array_tasks_open, $array_tasks_close);
...
//							$overdue_companies[] = $empty;
            ...
            $close_tasks_info["overdue_tasks"] = $overdue_tasks;
//			$close_tasks_info["overdue_companies"] = $overdue_companies;
    ...
    for ($i = 0; $i < count($info_about_all_tasks); $i++) {
        foreach ($open_count_tasks as $info) {
            if (($info["class_error_name"] == $info_about_all_tasks[$i]["class_error_name"]) && ($info["type_problem_name"] == $info_about_all_tasks[$i]["type_problem_name"])) {
                if ($info["count"] > 0) {
