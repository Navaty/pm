...
foreach($info as $inf) {
	...
	if(is_array($inf["overdue_tasks"])) {
			...
			$number_of_fault_company = 0;
			foreach($timework["companiesworktime"] as $item_worktime) {
//				echo $item_worktime["name"].' работал с задачей '.round( (($item_worktime["time"])/(60*60)), 2, PHP_ROUND_HALF_UP).' ч.'.PHP_EOL, "<br/>";
				if($item_worktime["time"] > 86400) {
					$number_of_fault_company++;
					$tasks_excel .= $item_worktime["name"].' просрочил на '.round( (($item_worktime["time"])/(60*60)), 2, PHP_ROUND_HALF_UP).' ч.'.PHP_EOL;
				}
			}
		if (!$number_of_fault_company)  foreach($timework["companiesworktime"] as $item_worktime) {
//                              echo $item_worktime["name"].' работал с задачей '.round( (($item_worktime["time"])/(60*60)), 2, PHP_ROUND_HALF_UP).' ч.'.PHP_EOL, "<br/>";
                                        $tasks_excel .= $item_worktime["name"].' просрочил на '.round( (($item_worktime["time"])/(60*60)), 2, PHP_ROUND_HALF_UP).' ч.'.PHP_EOL;
                        }
...
/*        $active_sheet->getStyle('B'.$schetchik_excel)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
	...
	$active_sheet->setCellValue('A'.$schetchik_excel, $inf["type_problem_name"]);
...
//echo json_encode('<p class="report_link">Отчет можно скачать по <a href="http://pm.citrt.net/plugins/report_terminals/autodetectexpiredtasks/'.$filename.'">этой</a> ссылке!</p>');
