<?php
function is_task_closed($ObjectId) {
  $sql = "SELECT completed_by_id FROM `og_project_tasks` WHERE id = '$ObjectId'";
  $res = mysql_query($sql);
  $arr = mysql_fetch_assoc($res);
  if($arr["completed_by_id"]>0) {
    echo "<font color='#FF9900' title='Завершенная задача!'><b>[закрыто]</b></font>";
  } else {
    echo "<font color='#00CC00' title='Не завершенная задача!'><b>[открыто]</b></font>";
  }
}
