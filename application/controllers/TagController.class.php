...
class TagController extends ApplicationController
{
    ...
    function list_tags()
    {
        ...
        $sazanCacheRes = sazanIsCached('list_tags', $inData, 60 * 60 * 24);
        if ($sazanCacheRes['status']) {
            $extra = array();
            $extra['tags'] = $sazanCacheRes['result'];
            $extra['sazanInfo'] = 'from_cache';
            $timeDiff = 'exists ';
            ...
            $extra = array();
            $extra['tags'] = $tags;
            sazanCache('list_tags', $inData, $tags);
            $timeDiff = 'from db ';
            $timeDiff .= microtime(1) - $startTime;
        }
        sazanCacheTime('list_tags', $inData, $timeDiff);
