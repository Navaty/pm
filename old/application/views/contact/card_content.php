<?php
function make_phone_contact($phone) {
  return "<input type='button' onclick=\"javascript: window.location.href = 'tel:$phone';\" value='Звонок через Lync'/>";
#\"tel:$phone\" target=_blank>call</a>";
}
	$contact = $object;
	$hasEmailAddrs = false;
?>
...
<?php } // if ?>
    ...
    <tr><td colspan=2><div style="font-weight:bold; font-size:120%; color:#888; border-bottom:1px solid #DDD;width:100%; padding-top:14px">
      ...
      <?php if($contact->getHPhoneNumber() || $contact->getHPhoneNumber2() || $contact->getHFaxNumber() || $contact->getHMobileNumber() || $contact->getHPagerNumber()) {?>
	     ...
	     <div><span><?php echo lang('hmobile') ?>:</span> <?php echo clean($contact->getHMobileNumber());?> <?php $mobphone = $contact->getHMobileNumber(); echo make_phone_contact($mobphone);?></div><?php } ?>
