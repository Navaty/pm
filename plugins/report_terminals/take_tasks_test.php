...
function get_info_about_task($idtask, $current_time) { // Получаем полную информацию по работе с задачей
	...
	$query_created_completed_on = "SELECT created_on, completed_on, assigned_to_user_id FROM `og_project_tasks` WHERE id = ".$idtask;
...
//	var_dump( $taskinfo["tasklive"]);
	$taskinfo["companiesworktime"] = get_info_about_workers_time($idtask, $current_time, $createtask, $completetask, give_me_id_organization_user($created_completed_on[1]["assigned_to_user_id"]));
	return $taskinfo;
}
